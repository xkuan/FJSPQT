...
def uni_instance_gen(n_j: int, n_m: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if total_assigned != n_m:
        raise ValueError("wrong capa group")
    ...
    for i in range(n_j):
        ...
        for j in range(n_m):
            ...
            if not avail:
                raise ValueError(f"empty capa group {capa_idx}")
...
if __name__ == "__main__":
    ...
    print("Processing times (different machines, unit: minutes):\n", op_ma_table)
    print("Queue times (Q-time, unit: minutes):\n", q_time)
    print("Lag times (Lag-time, unit: minutes):\n", lag_time)
